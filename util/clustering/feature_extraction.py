...
def _normalize_yake_features(features: list[str], threshold: float = 0.88, min_len: int = 4) -> list[str]:
    """
...
def extract_features(bp_df) -> pd.DataFrame:
    ...
    bp_df["features"] = bp_df.apply(lambda row: _create_feature_vector(row, max_features=max_features, max_keywords=max_keywords, max_tags=max_tags), axis=1)
